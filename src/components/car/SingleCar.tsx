    ...
    return (
        <Pressable onPress={() => SelectCar(id)} className='rounded-lg border border-border bg-btnBorder overflow-hidden' style={{ width: 186, height: 265 }}>
            <View className="flex-row  items-end mt-2  px-6 justify-end">
                <View className={` ${isFav(id) ? "bg-bgTab" : "bg-white"} justify-center items-center rounded-full`} style={{ height: 30, width: 30 }}><FontAwesome name={isFav(id) ? "heart" : "heart-o"} size={18} color={isFav(id) ? "red" : "#767676"} /> </View>
            </View>
            ...
            <View className='flex-1 bg-white gap-2 px-2'>
                ...
                <View className='flex-row justify-between items-center'>
                    <View className="flex-row gap-1 items-center">
                        ...
                        <Text className="text-[9px] text-placeholder">{seats} seats</Text>
                    </View>
